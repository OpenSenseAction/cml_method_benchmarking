# code from https://github.com/OpenSenseAction/OPENSENSE_sandbox/blob/main/notebooks/opensense_data_downloader_and_transformer.py
...
def transform_andersson_openMRG_city_gauges(data_fn, metadata_fn):
    ...
    return ds_gauges_city_org

def transform_andersson_openMRG_SMHI_gauge(data_fn):
    ...
    return ds_gauges_smhi

import pyproj
def transform_andersson_openMRG_radar(data_fn):
...
def wgs84_to_psp_radolan(lon, lat):
    # used to derive poin from radar
    R = 6370040.0  # earth radius in mm
    lon_0 = np.deg2rad(10.0)  # 10 Grad Ost im Bogenmass, Zentral-Meridian
    lat_0 = np.deg2rad(60.0)  # 60 Grad Nord im Bogenmass, True-Scale-Breite

    # decimal degree in radians
    lon = np.deg2rad(lon)
    lat = np.deg2rad(lat)

    # stereographic scaling factor
    M = (1.0 + np.sin(lat_0)) / (1.0 + np.sin(lat))

    # new coordinates in polarstereographic projection
